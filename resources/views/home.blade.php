...
<button id="back-to-top"
        ...
        class="fixed bottom-8 right-8 z-50 w-11 h-11 rounded-full
               ...
               hover:bg-[#5B8C2A] transition-all duration-300">
...
<style>
    ...
    .bg-hero-1 { object-position: center center; }
    .bg-hero-2 { object-position: center center; }
...
<div id="nav-wrapper" class="fixed top-0 inset-x-0 z-[90] flex justify-center px-4 md:px-6 pt-6 md:pt-8 transition-all duration-500 pointer-events-none">
    ...
    <nav id="main-nav" class="pointer-events-auto flex justify-between items-start md:items-start w-full max-w-full px-4 py-2 transition-all duration-[600ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] rounded-full border border-transparent">
        ...
        <a href="{{ url('/#home') }}" id="nav-logo-link" class="flex items-center md:items-start group transition-all duration-500 w-auto md:w-[180px]">
            <img src="{{ asset('images/logo.webp') }}" alt="Bali Coconut Art" class="logo-shadow w-16 h-16 md:w-[100px] md:h-[100px] object-contain group-hover:scale-105 transition-all duration-500 brightness-0 invert">
        </a>

        {{-- HAMBURGER BUTTON (MOBILE ONLY) --}}
        <div class="flex md:hidden items-center ml-auto mt-2">
            <button id="mobile-menu-btn" class="text-white hover:text-[#8DC641] transition-colors p-2 focus:outline-none">
        ...
        <div id="nav-menu" class="hidden md:flex flex-1 justify-center items-center gap-2 lg:gap-4 text-sm md:text-base font-bold tracking-wide -mt-2 transition-all duration-500">
            <a href="{{ url('/#home') }}" class="menu-link px-4 py-2 rounded-full text-white hover:text-white hover:bg-white/20 transition-all duration-300">Home</a>
            <a href="{{ url('/about') }}" class="menu-link px-4 py-2 rounded-full text-white hover:text-white hover:bg-white/20 transition-all duration-300">About</a>
            <a href="{{ route('services') }}" class="menu-link px-4 py-2 rounded-full text-white hover:text-white hover:bg-white/20 transition-all duration-300">Services</a>            
            <a href="{{ route('gallery') }}" class="menu-link px-4 py-2 rounded-full text-white hover:text-white hover:bg-white/20 transition-all duration-300">Gallery</a>
        </div>
...
<div id="mobile-menu-wrapper" class="fixed inset-0 z-[100] flex justify-end opacity-0 pointer-events-none transition-opacity duration-500">
    ...
    <div id="mobile-menu-panel" class="relative w-[80%] max-w-sm h-full bg-white shadow-[-20px_0_40px_rgba(0,0,0,0.15)] flex flex-col pt-10 px-8 transform translate-x-full transition-transform duration-[600ms] ease-[cubic-bezier(0.25,1,0.5,1)] rounded-l-[2rem] overflow-hidden">
        ...
        {{-- Menu Links (Dengan efek interaktif garis hijau) --}}
        <div class="flex flex-col gap-1 mt-4">
            <a href="{{ url('/#home') }}" class="mobile-link group relative flex items-center text-gray-900 font-Inter text-l tracking-[0.15em] uppercase py-4 transition-all duration-300 hover:text-[#5B8C2A]">
                <span class="w-0 h-[2px] bg-[#5B8C2A] transition-all duration-300 group-hover:w-6 mr-0 group-hover:mr-3 inline-block"></span>
                Home
            </a>
...
<div id="hero-bg-container" class="absolute inset-0 w-full h-full bg-gray-900 pointer-events-none overflow-hidden z-0">
        {{-- HANYA 2 BACKGROUND --}}
        <picture class="hero-bg-slide bg-hero-1 absolute inset-0 w-full h-full select-none transition-opacity duration-1000 ease-in-out opacity-100 animate-gentle-drift">
            <source media="(max-width: 767px)" srcset="{{ asset('images/day1.webp') }}">
            <img src="{{ asset('images/new2.webp') }}" class="w-full h-full object-cover object-center" alt="Background 1">
        </picture>

        <picture class="hero-bg-slide bg-hero-2 absolute inset-0 w-full h-full select-none transition-opacity duration-1000 ease-in-out opacity-0">
            <source media="(max-width: 767px)" srcset="{{ asset('images/day2.webp') }}">
            <img src="{{ asset('images/new1.webp') }}" class="w-full h-full object-cover object-center" alt="Background 2">
        </picture>
    ...
    {{-- Hero Content --}}
        ...
        <h1 class="text-[2.4rem] sm:text-5xl lg:text-[4.5rem] font-extrabold text-white uppercase leading-[1.15] mb-6 drop-shadow-2xl tracking-tight">
            ...
            <span class="inline-flex items-center gap-2 sm:gap-3 whitespace-nowrap">
                <span class="font-serif italic font-normal lowercase text-white text-[1.8rem] sm:text-4xl lg:text-[3.5rem] tracking-normal align-middle drop-shadow-lg">
                    for
                </span>
                <span class="text-[#8CC63F] align-middle tracking-[2px] [text-shadow:0_0_15px_rgba(140,198,63,0.8)] [-webkit-text-stroke:0.3px_#FFFFFF]">
                    UNFORGETTABLE
                </span>
            </span>
            <br>
            ...
            {{-- Subtitle --}}
            <p class="font-sans text-white text-base md:text-[1.15rem] pr-4 md:pr-0 max-w-[300px] sm:max-w-lg mb-8 md:mb-10 leading-relaxed font-normal">
        ...
        <div class="bg-white rounded-[2rem] shadow-[0_15px_50px_rgba(0,0,0,0.08)] max-w-6xl w-full py-8 md:py-12 px-4 md:px-8 grid grid-cols-2 gap-y-8 gap-x-2 md:flex md:flex-row md:justify-around items-start md:items-center border border-gray-50">
            ...
            <div class="group relative flex flex-col items-center text-center px-2 py-4 md:py-6 w-full md:w-1/4 cursor-pointer z-10">
                ...
                <div class="relative w-12 h-12 md:w-14 md:h-14 mb-4">
                    {{-- Efek miring di belakang --}}
                    <div class="absolute inset-0 bg-[#8CC63F]/40 rounded-2xl opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:rotate-6 group-hover:translate-x-0.5 group-hover:translate-y-0.5"></div>
                    {{-- Ikon Utama --}}
                    <div class="absolute inset-0 bg-[#8CC63F]/15 text-[#8CC63F] flex items-center justify-center text-xl md:text-2xl rounded-2xl transition-all duration-300 group-hover:bg-[#8CC63F] group-hover:text-white shadow-sm"><i class="fas fa-stamp"></i></div>
                </div>
            ...
            <div class="group relative flex flex-col items-center text-center px-2 py-4 md:py-6 w-full md:w-1/4 cursor-pointer z-10">
                ...
                <div class="relative w-12 h-12 md:w-14 md:h-14 mb-4">
                    <div class="absolute inset-0 bg-[#8CC63F]/40 rounded-2xl opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:rotate-6 group-hover:translate-x-0.5 group-hover:translate-y-0.5"></div>
                    <div class="absolute inset-0 bg-[#8CC63F]/15 text-[#8CC63F] flex items-center justify-center text-xl md:text-2xl rounded-2xl transition-all duration-300 group-hover:bg-[#8CC63F] group-hover:text-white shadow-sm"><i class="fas fa-user-tie"></i></div>
                </div>
            ...
            <div class="group relative flex flex-col items-center text-center px-2 py-4 md:py-6 w-full md:w-1/4 cursor-pointer z-10">
                ...
                <div class="relative w-12 h-12 md:w-14 md:h-14 mb-4">
                    <div class="absolute inset-0 bg-[#8CC63F]/40 rounded-2xl opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:rotate-6 group-hover:translate-x-0.5 group-hover:translate-y-0.5"></div>
                    <div class="absolute inset-0 bg-[#8CC63F]/15 text-[#8CC63F] flex items-center justify-center text-xl md:text-2xl rounded-2xl transition-all duration-300 group-hover:bg-[#8CC63F] group-hover:text-white shadow-sm"><i class="fas fa-truck"></i></div>
                </div>
            ...
            <div class="group relative flex flex-col items-center text-center px-2 py-4 md:py-6 w-full md:w-1/4 cursor-pointer z-10">
                ...
                <div class="relative w-12 h-12 md:w-14 md:h-14 mb-4">
                    <div class="absolute inset-0 bg-[#8CC63F]/40 rounded-2xl opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:rotate-6 group-hover:translate-x-0.5 group-hover:translate-y-0.5"></div>
                    <div class="absolute inset-0 bg-[#8CC63F]/15 text-[#8CC63F] flex items-center justify-center text-xl md:text-2xl rounded-2xl transition-all duration-300 group-hover:bg-[#8CC63F] group-hover:text-white shadow-sm"><i class="fas fa-glass-cheers"></i></div>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('scroll', () => {
        if (window.scrollY > 50) {
            ...
            logoImg.classList.remove('w-16', 'h-16', 'md:w-[100px]', 'md:h-[100px]', 'invert', 'logo-shadow');
            ...
            navLinks.forEach(link => {
                ...
                link.classList.add('text-gray-800', 'hover:text-[#5B8C2A]', 'hover:bg-gradient-to-r', 'hover:from-[#D0E9A8]/40', 'hover:to-[#78B83A]/40');
            });
        } else {
            ...
            logoImg.classList.add('w-16', 'h-16', 'md:w-[100px]', 'md:h-[100px]', 'invert', 'logo-shadow');
            ...
            navLinks.forEach(link => {
                link.classList.remove('text-gray-800', 'hover:text-[#5B8C2A]', 'hover:bg-gradient-to-r', 'hover:from-[#D0E9A8]/40', 'hover:to-[#78B83A]/40');
    ...
    // --- 3 & 4. BACKGROUND SLIDER DAN CARD SLIDER (SINKRON) ---
    ...
    function updateMainSlide(index) {
        ...
        if (heroTextContent) {
            ...
            void heroTextContent.offsetWidth; 
    ...
    // EVENT LISTENER UNTUK DOTS BACKGROUND
    dots.forEach((dot, index) => {
        dot.addEventListener('click', () => {
            currentMainIndex = index;
            updateMainSlide(currentMainIndex);
            
            // SINKRONISASI: Card ikut bergeser jika dot diklik
            nextCardSlide();
            resetAutoSlide();
        });
    });
    ...
    // EVENT LISTENER UNTUK TOMBOL CARD MANUAL
    cardNext.addEventListener('click', () => {
        nextCardSlide();
        // Reset timer utama agar tidak bertabrakan jika user sedang klik-klik card
        resetAutoSlide(); 
    });
